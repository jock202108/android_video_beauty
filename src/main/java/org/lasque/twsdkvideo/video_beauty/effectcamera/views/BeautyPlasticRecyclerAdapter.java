...
public class BeautyPlasticRecyclerAdapter extends RecyclerView.Adapter<BeautyPlasticRecyclerAdapter.BeautyViewHolder>{
    ...
    public BeautyViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.lsq_recycler_beauty_item_layout_new,null);
        BeautyViewHolder viewHolder = new BeautyViewHolder(view);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(final BeautyViewHolder beautyViewHolder, @SuppressLint("RecyclerView") final int position) {

        String code = mBeautyParams.get(position);
        code = code.toLowerCase();

        if (position == 0){
            beautyViewHolder.mPoint.setVisibility(View.VISIBLE);
        } else {
            beautyViewHolder.mPoint.setVisibility(View.GONE);
        }
...
//        if (selected){
//            beautyViewHolder.mBeautyLevelImage.setBackgroundResource(R.drawable.circle_bg_22);
//        } else {
//            beautyViewHolder.mBeautyLevelImage.setBackgroundResource(R.drawable.circle_bg_26);
//        }
    ...
    class BeautyViewHolder extends RecyclerView.ViewHolder{
        ...
        public BeautyViewHolder(View itemView) {
            ...
            mPoint = itemView.findViewById(R.id.lsq_plastic_point);
        }
    }
}
