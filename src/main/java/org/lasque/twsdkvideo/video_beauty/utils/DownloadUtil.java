...
public class DownloadUtil {
    ...
    public void download(final String url, final String saveDir, final String saveName, final OnDownloadListener listener) {
        ...
        AppConstants.callList.add(call);
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                // 储存下载文件的目录
                String savePath = isExistDir(mContext, saveDir);
                try {
                    is = response.body().byteStream();
                    long total = response.body().contentLength();
                    File file = new File(savePath, saveName);
                    long totalSize = new SpUtils(mContext).getLong(file.getAbsolutePath(), 0);
                    if (totalSize == 0) {
                        new SpUtils(mContext).putLong(file.getAbsolutePath(), total);
                    }
    ...
    public static String isExistDir(Context context, String saveDir) throws IOException {
        // 下载位置
        File downloadFile = new File(context.getFilesDir(), saveDir);
