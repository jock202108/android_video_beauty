...
public class TuSdkMovieScrollView extends FrameLayout  {
    ...
    public void setOutLineType(int outLineType) {
        mMovieScrollContentView.setOutlineType(outLineType);
    }
    ...
    private void init(Context context) {
        ...
        mMovieScrollContentView.setNeedShowCursor(true);
        mMovieScrollContentView.setCursorListHeight(TuSdkContext.dip2px(32));
        mMovieScrollContentView.setCursorListTopMargin(TuSdkContext.dip2px(5));
...
//        setHorizontalScrollBarEnabled(false);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (!isSetLayoutWidth) {
            FrameLayout.LayoutParams layoutParams1 = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getHeight());
...
//        }
        if (mType == 1) {
            TuSdkRangeSelectionBar rangeSelectionBar = (TuSdkRangeSelectionBar) mMovieScrollContentView.getChildAt(2);
            int barWidth = TuSdkContext.dip2px(11);
            if (rangeSelectionBar != null) barWidth = rangeSelectionBar.getBarWidth();
            mMovieScrollContentView.setPadding(1, 0,  1, 0);
            isInit.getAndAdd(-1);
        } else {
            mMovieScrollContentView.setPadding(1, 0, 1, 0);
            isInit.getAndAdd(-1);
...
//    }
    ...
    public void addFirstFrameBitmap(Bitmap bitmap) {
        mMovieScrollContentView.addFirstFrameBitmap(bitmap);
    }
    ...
    public void seekTo(float percent) {
        mMovieScrollContentView.setPercent(percent);
        mPercent = percent;
        if (isTouching) prePercent = mPercent;
        if (mMovieScrollContentView != null && isAddColorRect) {
...
//        }
    ...
    /** 光标最左边 **/
    public void setCursorMinPercent(float minPercent) {
        mMovieScrollContentView.setCursorMinPercent(minPercent);
    }

    /** 光标最右边 **/
    public void setCursorMaxPercent(float maxPercent) {
        mMovieScrollContentView.setCursorMaxPercent(maxPercent);
    }
    /** 光标位置 **/
    public void setPercent(float maxPercent) {
        mMovieScrollContentView.setPercent(maxPercent);
    }
    /**
     * 处理父View （ViewPager） 对光标和裁剪条的事件拦截
     *
     * @param
     */
    @Override
...
//    }
    ...
    public void setOnPlayPointerChangeListener(TuSdkMovieScrollContent.OnPlayProgressChangeListener progressChangeListener) {
        ...
        mMovieScrollContentView.setProgressChangeListener(progressChangeListener);
    }
}
