...
import org.lasque.twsdkvideo.video_beauty.views.editor.playview.rangeselect.TuSdkMovieGrayView;
...
//                }
    ...
    private boolean calcPercentRange(int touchType,boolean notifiy,float diffX) {
        ...
        //超过最大值
        if (mCenterFrame.getWidth()+1 >= maxLength) {//滑动中等于的可能性太小。。
            return handleMax(touchType, diffX);
        }
        //超过最小值
        if (mCenterFrame.getWidth()-1 <= minLength) {//滑动中等于的可能性太小。。浮点运算精确度的原因
