...
import org.lasque.twsdkvideo.video_beauty.views.editor.ruler.HorizontalPlayViewOne;
import org.lasque.twsdkvideo.video_beauty.views.editor.ruler.PlayLineView;
import org.lasque.twsdkvideo.video_beauty.views.editor.ruler.RangeSelectionBar;

import java.util.ArrayList;
import java.util.List;

import static org.lasque.twsdkvideo.video_beauty.views.editor.LineView.LineViewType.CanRolling;
import static org.lasque.twsdkvideo.video_beauty.views.editor.LineView.LineViewType.DrawColorRect;
...
public class LineView extends FrameLayout {
    ...
    public void loadView(){
        ...
        //DrawColorRect
        if (LineViewType.DrawColorRect == mLineViewType){
...
//                    selectionBarHeight = height;
        ...
        if (LineViewType.CanRolling == mLineViewType){
...
//                        mRangeSelectionBar.setValue(mTotalWidth, selectionBarHeight, 0);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if(mLineViewType == LineViewType.CanRolling || mLineViewType == LineViewType.DrawColorRect) {
            horizontalPlayViewOne.setInitValue(mBitmapList, mTotalTimeUs, mScreenWidth / 2 - mInParentLeft,
                    mPointerColorId, mCoverOutBorderColor);
        ...
        if(horizontalPlayViewOne!=null && (mLineViewType == LineViewType.DrawColorRect || mLineViewType == LineViewType.CanRolling ))horizontalPlayViewOne.setLeftMargin(mScreenWidth/2 - mInParentLeft);
        if(mRangeSelectionBar!=null && (mLineViewType == LineViewType.DrawColorRect || mLineViewType == LineViewType.CanRolling ))mRangeSelectionBar.setStartLineX(mScreenWidth/2 - mInParentLeft);
    }
