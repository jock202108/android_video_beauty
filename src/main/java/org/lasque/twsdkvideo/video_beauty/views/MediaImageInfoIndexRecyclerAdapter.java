...
public class MediaImageInfoIndexRecyclerAdapter extends RecyclerView.Adapter<MediaImageInfoIndexRecyclerAdapter.MediaInfoIndexViewHolder> {
    ...
    public MediaImageInfoIndexRecyclerAdapter(Context mContext) {
    ...
    public void setData(List<String> indexTitles, Hashtable<String, List<ImageSqlInfo>> htImg){
    ...
    @Override
    public void onBindViewHolder(@NonNull MediaInfoIndexViewHolder holder, int position) {
        if(indexTitles != null && indexTitles.size() > 0) {
            String indexTitleName = indexTitles.get(position);
            List<ImageSqlInfo> mediaList = getMediaList(indexTitleName);
            ImageSqlInfo imageSqlInfo = mediaList.get(0);

            if(imageSqlInfo != null) {
                String path;
                if (!TextUtils.isEmpty(path = imageSqlInfo.path)) {
                    Glide.with(context).load(path).into(holder.mPreImageview);
                }
            }
            holder.mTitleCountView.setText(mediaList != null ? (mediaList.size() + "") : "");
            holder.mTitleCountView.setTextColor(VideoBeautyPlugin.themeMode == 0? ContextCompat.getColor(mContext,R.color.color_6E7187): ContextCompat.getColor(mContext,R.color.color_787a8c));


            holder.mTitleView.setText(indexTitleName);
            holder.mTitleView.setTextColor(VideoBeautyPlugin.themeMode == 0? ContextCompat.getColor(mContext,R.color.lsq_color_black): ContextCompat.getColor(mContext,R.color.lsq_color_white));
          
            holder.line.setBackgroundColor(DarkModeUtils.getColor(mContext,R.color.color_E8E9EF,R.color.color_373A4E));
