 ...
 * @Copright (c) 2017 tw. All rights reserved.
 *
 */
...
import org.lasque.twsdkvideo.video_beauty.views.ConfigViewParams.ConfigViewArg;
...
//			layoutParams.leftMargin =(int)Math.floor((progress*(getSeekbar().getWidth()-300*progress)));
			this.getConfigValueView().setText((int)(progress*200)+"%");
		}
		if (mDelegate != null)
		{
	...
	public void setConfigViewArg(ConfigViewArg arg)
	{
		...
		{
//			RelativeLayout.LayoutParams layoutParams = (LayoutParams) getConfigValueView().getLayoutParams();
//			layoutParams.leftMargin =(int)Math.floor((arg.getPercentValue()*mSeekbar.getWidth()));
			this.getConfigValueView().setText((int)(arg.getPercentValue()*200)+"%");
		}
	...
	public void setProgress(float progress)
	{
		...
		if(this.getConfigValueView()!=null)
		{
			...
			if(mSeekbar.getWidth() != 0){
				layoutParams.leftMargin =(int)Math.floor((progress*(getSeekbar().getWidth()-60*progress)));
			}else {
				layoutParams.leftMargin =(int)Math.floor((progress*(TuSdkContext.getScreenSize().width-TuSdkContext.dip2px(128))));
			}
