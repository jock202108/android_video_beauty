...
public class TabPagerIndicator extends LinearLayout {
    ...
    private Map<String, Float> hmTitle;

    private List<String> stickerGroupCategories;
    ...
    public void setTabItems(List<String> stickerGroupCategories) {
        if (stickerGroupCategories == null)
            return;
        this.stickerGroupCategories = stickerGroupCategories;
        this.removeAllViews();
    ...
    private void getAttrs(Context context, AttributeSet attributeSet) {
        ...
        fixedWidth = attributes.getBoolean(R.styleable.TabPagerIndicator_fixed_width, false);
        ...
        mLineHeight = TuSdkContext.dip2px(mLineHeight);
        itemWidth = TuSdkContext.dip2px(attributes.getInt(R.styleable.TabPagerIndicator_item_width, 0)) ;
    ...
    protected void dispatchDraw(Canvas canvas) {
        if (fixedWidth) {
            ...
            canvas.save();
            String tabTitle;
            if (stickerGroupCategories != null && (null != (tabTitle = stickerGroupCategories.get(mCurrentPosition)))) {
                float textWidth;
                if (hmTitle != null && ((textWidth = hmTitle.get(tabTitle)) != 0)) {
                    canvas.drawLine(mMoveX + (mLineWidth - textWidth) / 2, getHeight(), mMoveX + mLineWidth - (mLineWidth - textWidth) / 2, getHeight(), mLinePaint);
                }
                canvas.restore();
                super.dispatchDraw(canvas);
            }
    ...
    private View generateTitleView(String title) {
        ...
        params.width = itemWidth==0?getScreenWidth() / mDefaultVisibleCounts: dip2px(66);
        ...
        float tvWidth = tv.getPaint().measureText(tv.getText().toString());
        if (hmTitle != null) hmTitle.put(title, tvWidth);
    ...
    //private int mCurrentPosition = -1; 原代码从-1开始会造成220line没滑动赋值之前 数组越界
